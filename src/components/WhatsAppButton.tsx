...
export const WhatsAppButton: React.FC<WhatsAppButtonProps> = ({
  ...
  return (
    <div
      ...
      className="fixed bottom-5 right-4 sm:right-5 z-40 flex flex-col items-end select-none"
    >
      ...
      {isOpen && (
        <div
          ...
          className="mb-3 w-[320px] sm:w-[360px] max-w-[calc(100vw-2rem)] bg-[#0E0E0E] border border-emerald-500/40 rounded-sm shadow-[0_15px_40px_rgba(0,0,0,0.85)] text-zinc-200 overflow-hidden animate-in slide-in-from-bottom-5 fade-in duration-200"
        >
          {/* Header */}
          <div className="bg-[#121212] px-4 py-3.5 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <div className="flex items-center gap-1.5">
                  <h4 id="whatsapp-popup-title" className="font-heading text-xs font-bold text-white uppercase tracking-wider truncate">
                    Aura Global Industries
                  </h4>
                  <span className="text-[9px] bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-1 py-0.2 rounded-sm font-mono uppercase font-bold shrink-0">
                    Direct
                  </span>
                </div>
                <p className="text-[10px] text-zinc-400 font-mono flex items-center gap-1 truncate">
                  <MapPin className="w-3 h-3 text-[#D4AF37] shrink-0" aria-hidden="true" /> Nazimabad, Karachi • Factory Online
                </p>
              </div>
            </div>
            ...
            <button
              ...
              className="p-1 rounded-sm text-zinc-500 hover:text-white hover:bg-white/10 transition-colors cursor-pointer shrink-0"
          ...
          <div className="p-4 space-y-3.5 bg-[#090909]">
            ...
            <div className="p-2.5 rounded-sm bg-[#141414] border border-white/5 space-y-1">
              ...
              <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono pt-1 flex-wrap gap-1">
                <span className="flex items-center gap-1">
                  <Clock className="w-3 h-3 text-emerald-400" aria-hidden="true" /> Avg. reply: Under 15 mins
                </span>
                <span className="text-[#D4AF37] flex items-center gap-0.5">
                  <ShieldCheck className="w-3 h-3" aria-hidden="true" /> Verified Factory
                </span>
              </div>
            </div>
              ...
              <div className="flex flex-col gap-1.5">
                {presetMessages.map((preset, idx) => (
                  ...
                  >
                    ...
                    <ArrowUpRight className="w-3 h-3 text-zinc-500 group-hover:text-emerald-400 shrink-0" aria-hidden="true" />
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-1.5 pt-1">
              <label htmlFor="whatsapp-custom-message" className="block text-[10px] font-mono uppercase text-zinc-400">
      ...
      <div className="relative group flex flex-row-reverse items-center gap-3">
        <button
          ...
          className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full bg-[#0E0E0E] border-2 border-emerald-500/70 hover:border-emerald-400 flex items-center justify-center text-white shadow-[0_0_25px_rgba(16,185,129,0.35)] hover:shadow-[0_0_35px_rgba(16,185,129,0.6)] transition-all duration-300 cursor-pointer group-hover:scale-105"
          aria-label={isOpen ? 'Close WhatsApp Chat' : 'Open WhatsApp Chat with Aura Global Industries'}
        >
          {/* Concentric Radar Ping Waves */}
          <span className="absolute -inset-1.5 rounded-full bg-emerald-500/20 animate-ping opacity-75 pointer-events-none" aria-hidden="true" />
          <span className="absolute -inset-3 rounded-full border border-emerald-500/30 animate-pulse pointer-events-none" aria-hidden="true" />

          {/* Metallic Inner Ring */}
          <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-gradient-to-tr from-emerald-600 via-emerald-500 to-emerald-400 flex items-center justify-center shadow-inner">
            <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6 text-white fill-white transition-transform group-hover:scale-110 duration-200" aria-hidden="true" />
          </div>
